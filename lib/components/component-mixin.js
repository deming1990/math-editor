  ...
  methods: {
    ...
    _format() {
      ...
      deeps.forEach((nodeDeep) => {
        ...
        if(node && node.$el) {
          node.$el.style.transform = `translate3d(0,${top}px,0)`
        }
      })
      if(NodeManager.isUnaryOperatorNode(this.model) 
        ...
        if(this.hasPostfix) {
          this.$refs.operatorNodePostfix.style.transform = `translate3d(0,${top}px,0)`
        }
        if(this.hasPrefix) {
          this.$refs.operatorNodePrefix.style.transform = `translate3d(0,${top}px,0)`
        }
      }
    },
    _getNodeDeep(node) {
