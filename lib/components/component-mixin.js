...
export default {
  methods: {
    ...
    addFormatListener(target) {
      this.formatObserver = this.addMutationObserver(target, this.format)
    },
    removeFormatListener() {
      this.removeMutationObserver(this.formatObserver)
    },
    addMutationObserver(target, callback) {
    ...
    getNodeDeep(node) {
      ...
      superScriptDeep = superScriptDeep && (superScriptDeep * (20/28) - (10/28))
