...
class BoundaryDetection {
  ...
  detectBoundary(rowModel, rowContainer, changedTargets, fontSize) {
      ...
      while(count < length) {
        ...
        if(totalWidth > maxWidth) {
          if(nodeModel.compType === NODE_TYPES.TEXT_NODE) {
            ...
            if(count > 0) {
              const preNodeModel = rowModel.children[count - 1]
              return {
                cursorPosition: (preNodeModel.value && preNodeModel.value.length) || 0,
                nodeModel: preNodeModel
              }
            } else {
              return {
                cursorPosition: 0,
